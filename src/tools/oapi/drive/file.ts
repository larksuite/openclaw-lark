 ...
 *
 * Actions: create_folder, list, get_meta, copy, move, delete, upload, download
 *
...
import type { DriveFileListData, DriveFileData, DriveFolderData, DriveTaskData } from '../sdk-types';
...
const FeishuDriveFileSchema = Type.Union([
  ...
  Type.Object({
    ...
    folder_token: Type.String({
      description: '父文件夹 token（必填）。在该文件夹下创建子文件夹。注意：该接口不支持并发创建，调用频率上限为 5QPS',
    }),
  }),
  ...
  api.registerTool(
    {
      ...
      description:
        ...
        '\n\nActions:' +
        '\n- create_folder（创建文件夹）：在指定文件夹下创建子文件夹。需要 name 和 folder_token' +
      ...
      async execute(_toolCallId: string, params: unknown) {
        ...
        try {
          ...
          switch (p.action) {
            ...
            // -----------------------------------------------------------------
            case 'create_folder': {
              log.info(`create_folder: name=${p.name}, folder_token=${p.folder_token}`);
              ...
              const data = res.data as DriveFolderData | undefined;
              log.info(`create_folder: new folder token=${data?.token}`);
