 ...
 * ownerPolicy=strict 时仅当配对用户 === 应用 owner 时触发。
 * ownerPolicy=multiUser 时允许绑定用户触发。
 *
...
import { getOwnerPolicyMode } from '../core/owner-policy';
 ...
 *
 * 流程：
 *   1. strict 模式检查 userOpenId === 应用 owner；multiUser 模式允许绑定用户触发
...
export async function triggerOnboarding(params: {
  ...
  if (getOwnerPolicyMode(acct) === 'strict') {
    ...
    log.info(`ownerPolicy=multiUser, user ${userOpenId} starting OAuth`);
  }
