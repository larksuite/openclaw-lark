...
function _optimizeMarkdownStyle(text: string, cardVersion = 2): string {
  ...
  if (cardVersion >= 2) {
    ...
    // ── 4 (v1 / 原生 post). 表格与相邻段落之间补「真正的空行」──────────
    // 原生 post(tag:md) 走 GFM：表格紧贴上/下一行普通文本时，飞书会把那行
    // lazy-continuation 并进表格（"表格 + 一句备注" 场景被吞成畸形表格行）。
    // 这里补 `\n\n`（而非 v2 的 `<br>`，后者会污染原生 markdown）。在还原
    // 代码块之前处理，避免代码块内的竖线行被误判为表格行。
    // 4a. 表格前是普通文本（非表格行）时，插入一个空行
    r = r.replace(/^([^|\n].*)\n(\|.+\|)/gm, '$1\n\n$2');
    // 4b. 表格最后一行紧跟非表格、非空行时，插入一个空行。
    //     只匹配「表格行 → 紧随的非表格行」这个边界（单行 + 零宽前瞻），
    //     不重复整块，避免大表格在文末时 `(?:…\n)+` 回溯导致的 O(n²)。
    //     表格内部行后跟的是另一表格行（前瞻 `|` 失败），不会误插。
    r = r.replace(/^(\|.+\|[^\S\n]*)\n(?=[^|\n])/gm, '$1\n\n');
